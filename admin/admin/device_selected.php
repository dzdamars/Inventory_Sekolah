 ...
 <br/>
	...
	<div class="pull-left">
			  ...
			  <?php	
	           ...
	           $data=mysql_query("select * from device_name");
			   
			   while($value = mysql_fetch_array($data)){
			   		$count = mysql_query("select * from stdevice where dev_id = $value[dev_id]") or die(mysql_error());
			   		$count_row = mysql_num_rows($count);

			   		echo "<li class=''>
				<a href='device_selected.php?id=".$value['dev_id']."'>".$value['dev_name']."<span class='label label-default'style='margin-left:5px'>".$count_row."</span></a>
				</li>";
			   }
...
<?php
		...
		LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
		where stdevice.dev_id = $_GET[id]") or die(mysql_error());
